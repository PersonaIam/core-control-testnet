...
start () {
  ...
  if [ "$1" = "all" ]; then
    ...
    local fstatus=$(pm2status "${name}-forger-${network}" | awk '{print $13}')
    local rstatus=$(pm2status "${name}-relay-${network}" | awk '{print $13}')

    if [ "$rstatus" != "online" ]; then
      pm2 --name "${name}-relay-${network}" start $core/packages/core/bin/run -- relay:run --network $network --token $name > /dev/null 2>&1
    else
    ...
    if [ "$secrets" = "[]" ]; then
      ...
      pm2 --name "${name}-forger-${network}" start $core/packages/core/bin/run -- forger:run --network $network --token $name > /dev/null 2>&1
    else
    ...
    local pstatus=$(pm2status "${name}-$1-${network}" | awk '{print $13}')

    if [[ "$secrets" = "[]" && "$1" = "forger" ]]; then
      echo -e "\n${red}Delegate secret is missing. Forger start aborted!${nc}"
    elif [ "$pstatus" != "online" ]; then
      pm2 --name "${name}-$1-${network}" start $core/packages/core/bin/run -- ${1}:run --network $network --token $name > /dev/null 2>&1
    else
...
restart () {
  ...
  if [ "$1" = "all" ]; then
    ...
    if [ "$rstatus" = "online" ]; then
      pm2 restart ${name}-relay-${network} > /dev/null 2>&1
    else
      echo -e "\n${red}Process relay not running. Skipping...${nc}"
    fi

    if [ "$fstatus" = "online" ]; then
      pm2 restart ${name}-forger-${network} > /dev/null 2>&1
    else
    ...
    if [ "$pstatus" = "online" ]; then
      pm2 restart ${name}-$1-${network} > /dev/null 2>&1
    else
...
stop () {
  ...
  if [ "$1" = "all" ]; then
    ...
    if [ "$rstatus" = "online" ]; then
      pm2 stop ${name}-relay-${network} > /dev/null 2>&1
    else
      echo -e "\n${red}Process relay not running. Skipping...${nc}"
    fi

    if [ "$fstatus" = "online" ]; then
      pm2 stop ${name}-forger-${network} > /dev/null 2>&1
    else
      echo -e "\n${red}Process forger not running. Skipping...${nc}"
    fi
    ...
    if [ "$pstatus" = "online" ]; then
      pm2 stop ${name}-$1-${network} > /dev/null 2>&1
    else
      echo -e "\n${red}Process $1 not running. Skipping...${nc}"
    fi
...
status () {
  ...
  echo -e -n "\n${cyan}${name}-core-${network}${nc} v${cyan}${corever}${nc} "
...
remove () {
  ...
  pm2 delete ${name}-forger-${network} > /dev/null 2>&1
  pm2 delete ${name}-relay-${network} > /dev/null 2>&1
  pm2 save > /dev/null 2>&1
...
update_info () {
  ...
  if [ -d $core ]; then
    ...
    echo -e -n "${cyan}${name}-core-${network}${nc} v${cyan}${corever}${nc} hash: ${cyan}${loc}${nc} status: "
